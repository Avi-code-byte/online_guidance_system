...
$sql = "SELECT * FROM employees WHERE loginid='$_POST[loginid]' AND password='$_POST[password]' AND Status='Enabled'";
	...
	{			
		...
		mysqli_query($conn,"UPDATE employees SET lastlogin='$dttim' where loginid='$_POST[loginid]'");
...
$sqlresult=mysqli_query($conn,$sqlins);
if(!$sqlresult)
{
	$regresi = 1;
	$regres = "<font color='red'><strong><h3>Failed to insert records</h3></strong></font>". mysqli_error($conn);
}
else
{
	$regresi = 1;
